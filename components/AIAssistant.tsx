...
const AIAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{role: 'user' | 'assistant', text: string}[]>([
    { role: 'assistant', text: "Yo! I'm the Flava25 Assistant. Craving something flame-grilled in Cradley Heath? Tell me what you're into and I'll find your perfect feast!" }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', text: recommendation || "Whoops, looks like I dropped a chip. Try asking that again!" }]);
    } catch (err) {
      setMessages(prev => [...prev, { role: 'assistant', text: "Something went wrong. My grill brain is overheating!" }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white w-80 md:w-96 rounded-3xl shadow-2xl border border-neutral-100 flex flex-col h-[500px] animate-slideInUp overflow-hidden">
          <div className="bg-neutral-900 p-6 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-white font-black uppercase text-sm tracking-tight">FLAVA AI</h4>
